...
function PedidoDetalle() {
  ...
  useEffect(() => {
    ...
    const storedPedido = localStorage.getItem(`pedido-${id}`);
    if (storedPedido) {
      const parsedPedido = JSON.parse(storedPedido);
      setPedido(parsedPedido);
      setStatus(parsedPedido.status || "Pendiente");
      setLoading(false);
    } else {
      fetchPedido();
    }
  ...
  const fetchPedido = async () => {
    try {
      ...
      localStorage.setItem(`pedido-${id}`, JSON.stringify(response.data));
    } catch (err) {
  ...
  const handleStatusChange = async (event) => {
    ...
    try {
      ...
      setPedido((prevPedido) => {
        const updatedPedido = { ...prevPedido, status: newStatus };
        localStorage.setItem(`pedido-${id}`, JSON.stringify(updatedPedido));
